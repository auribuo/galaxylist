...
public static partial class Extensions
{
	...
	private static bool IsInViewport(EquatorialCoordinate pos, Fov fov, EquatorialCoordinate viewportPos) =>
		viewportPos.RightAscention.ToDegree() - fov.Width / 2 < pos.RightAscention.ToDegree() &&
		viewportPos.RightAscention.ToDegree() + fov.Width / 2 > pos.RightAscention.ToDegree() &&
		viewportPos.Declination.ToDegrees() - fov.Height / 2 < pos.Declination.ToDegrees() &&
		viewportPos.Declination.ToDegrees() + fov.Height / 2 > pos.Declination.ToDegrees();
	...
	public static List<Viewport> CalculateViewports(
		this IEnumerable<Galaxy> source,
		Fov fov,
		Location? location = null,
		DateTime? time = null,
		double rasterApprox = 0.25
	)
	{
		var galaxies = source.ToList();
		...
		foreach (Galaxy galaxy in galaxies)
		{
			...
			catch (Exception)
			{
				...
				if (location is not null && time is not null)
				{
					...
					EquatorialCoordinate topLeft = (viewport.Pos.RightAscention.ToDegree() - fov.Width,
						viewport.Pos.Declination.ToDegrees() + fov.Height);

					EquatorialCoordinate topRight = (viewport.Pos.RightAscention.ToDegree() + fov.Width,
						viewport.Pos.Declination.ToDegrees() + fov.Height);

					EquatorialCoordinate bottomLeft = (viewport.Pos.RightAscention.ToDegree() - fov.Width,
						viewport.Pos.Declination.ToDegrees() - fov.Height);

					EquatorialCoordinate bottomRight = (viewport.Pos.RightAscention.ToDegree() + fov.Width,
						viewport.Pos.Declination.ToDegrees() - fov.Height);
